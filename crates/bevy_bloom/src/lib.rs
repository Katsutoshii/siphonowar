...
pub use settings::{BloomCompositeMode, BloomPrefilterSettings, CustomBloomSettings};
...
const BLOOM_SHADER_HANDLE: Handle<Shader> = Handle::weak_from_u128(929599476923909);

const BLOOM_TEXTURE_FORMAT: TextureFormat = TextureFormat::Rg11b10Float;

pub struct CustomBloomPlugin;

impl Plugin for CustomBloomPlugin {
    fn build(&self, app: &mut App) {
        load_internal_asset!(app, BLOOM_SHADER_HANDLE, "bloom.wgsl", Shader::from_wgsl);

        app.register_type::<CustomBloomSettings>();
        app.register_type::<BloomPrefilterSettings>();
        app.register_type::<BloomCompositeMode>();
        app.add_plugins((
            ExtractComponentPlugin::<CustomBloomSettings>::default(),
...
fn prepare_bloom_textures(
    ...
    views: Query<(Entity, &ExtractedCamera, &CustomBloomSettings)>,
) {
    for (entity, camera, settings) in &views {
        ...
        }) = camera.physical_viewport_size
        {
            ...
            let mip_height_ratio = settings.max_mip_dimension as f32 / height as f32;
...
fn compute_blend_factor(bloom_settings: &CustomBloomSettings, mip: f32, max_mip: f32) -> f32 {
